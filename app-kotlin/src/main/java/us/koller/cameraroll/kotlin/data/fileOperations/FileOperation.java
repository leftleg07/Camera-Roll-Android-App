package us.koller.cameraroll.kotlin.data.fileOperations;
...
import us.koller.cameraroll.kotlin.data.ContentObserver;
import us.koller.cameraroll.kotlin.data.Settings;
import us.koller.cameraroll.kotlin.data.models.AlbumItem;
import us.koller.cameraroll.kotlin.data.models.File_POJO;
import us.koller.cameraroll.kotlin.data.models.Video;
import us.koller.cameraroll.kotlin.util.ExifUtil;
import us.koller.cameraroll.kotlin.util.MediaType;
import us.koller.cameraroll.kotlin.util.StorageUtil;

import static us.koller.cameraroll.kotlin.data.fileOperations.FileOperation.Util.*;
import static us.koller.cameraroll.kotlin.util.Util.getLocale;
...
public abstract class FileOperation extends IntentService implements Parcelable {
    ...
    protected void onHandleIntent(Intent workIntent) {
        ...
        if (autoSendDoneBroadcast()) {
            if (pathsToScan.size() > 0) {
                onProgress(-1, -1);
                scanPaths(getApplicationContext(), new MediaScannerCallback() {
                    @Override
    ...
    public static Intent getDefaultIntent(Context context, int action, File_POJO[] files) {
        String actionString = getActionString(context, EMPTY);
        Class service = null;
//        switch (action) {
//            case COPY:
//                service = Copy.class;
...
//            }
    ...
    void scanPaths(final Context context, final MediaScannerCallback callback) {
